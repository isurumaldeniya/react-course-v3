...
const App = () => {
  ...
  async function fetchData() {
    try {
      ...
      setIsLoading(false);
      setJobs(jobs.concat(jobListNew));
      setCompany(jobs[0].company);

      companyList = [...new Set(jobs.map((job) => job.company))];
    } catch (error) {
      console.log(error);
    }
  }
    ...
    return (
      <section className="jobs-center">
        <div className="btn-container">
          {companyList.map((jobCompany, index) => {
            return (
              <button
                className={
                  jobCompany === company ? 'job-btn active-btn' : 'job-btn'
                }
                key={index}
                onClick={() => selectCompany(jobCompany)}
              >
                {jobCompany}
              </button>
            );
